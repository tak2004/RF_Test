                              ...
                              const TestResultCollector& Results)
{
  ...
  out.Open(URI, RF_SysFile::FileAccessMode::Write,
           RF_SysFile::FileAccessPriority::DelayReadWrite);
  if(out.CanWrite())
  {
    // start suites
    RF_Type::String tag("<testsuites>\n", sizeof("<testsuites>\n"));
    out.Write(reinterpret_cast<const RF_Type::UInt8*>(tag.c_str()), 0,
              tag.Length());
    for(RF_Type::UInt32 i = 0; i < Results.TestResults().Count(); ++i)
    {
      ...
      tag = RF_Type::String::Format(
          ...
          Results.TestResults()[i].TotalTime, "2007-11-02T23:13:49", 0, 0);
      out.Write(reinterpret_cast<const RF_Type::UInt8*>(tag.c_str()), 0,
                tag.Length());
      for(RF_Type::UInt32 j = 0;
          j < Results.TestResults()[i].TestResults.Count(); ++j)
      {
        // start testcase
        tag = RF_Type::String::Format(
